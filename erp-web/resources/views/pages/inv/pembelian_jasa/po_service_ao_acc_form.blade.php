...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <form method="POST" action="{{ URL::to('pembelian_jasa/approve_service') }}" class="form-horizontal r-separator">
                @csrf
                <input type="hidden" name="purchase_id" value="{{ $purchase['id'] }}" />
                <div class="text-right">
                    <a href="{{ URL::to('pembelian_jasa/service') }}"><button type="button" class="btn btn-danger btn-sm mb-2">Cancel</button></a>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row align-items-center mb-0" {{$role == 3 ? 'hidden' : ''}}>
                            <label class="col-sm-3 text-right control-label col-form-label">Signature Direktur</label>
                            <div class="col-9 border-left pb-2 pt-2">
                                <div class="row">
                                    <div class="col-sm-6">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="list_item" class="table table-bordered">
                                <thead>
                                    <tr>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            type: "GET",
            url: "{{ URL::to('pembelian_jasa/detail_service/' . $purchase['id']) }}", //json get site
            dataType: 'json',
            async: false,
            success: function(response) {
                ...
                for (i = 0; i < arrData.length; i++) {
                    // t.row.add([
                    //     '<div class="text-left"><input name="pd_id[]" type="hidden" value="'+arrData[i]['id']+'" /><input name="service_name[]" type="hidden" value="'+arrData[i]['service_name']+'" />'+arrData[i]['service_name']+'</div>',
                    ...
                    var tdAdd = '<tr>' +
                        ...
                        '</tr>';
                    $('#list_item').find('tbody:last').append(tdAdd);
    ...
    $("#list_item").on("click", ".removeOption", function(event) {
    ...
    saveSignatureButton1.addEventListener("click", function(event) {
        if (signaturePad1.isEmpty()) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ URL::to('pembelian_jasa/acc_ao_service_form/signature_holding/' . $purchase['id']) }}",
                dataType: 'json',
                data: {
                    _token: CSRF_TOKEN,
                    file: dataURL
    ...
    saveSignatureButton2.addEventListener("click", function(event) {
        if (signaturePad2.isEmpty()) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ URL::to('pembelian_jasa/acc_ao_service_form/signature_supplier/' . $purchase['id']) }}",
                dataType: 'json',
                data: {
                    _token: CSRF_TOKEN,
                    file: dataURL
    ...
    function saveSignatureDirector(){
        ...
        $.ajax({
            type: "POST",
            url: "{{ URL::to('pembelian_jasa/saveSignatureDirector/' . $purchase['id']) }}", //json get site
            dataType: 'json',
            data: {
                _token: CSRF_TOKEN,
                code : code
    ...
    function saveSignatureManager(){
        ...
        $.ajax({
            type: "POST",
            url: "{{ URL::to('pembelian_jasa/saveSignatureManager/' . $purchase['id']) }}", //json get site
            dataType: 'json',
            data: {
                _token: CSRF_TOKEN,
                code : code
