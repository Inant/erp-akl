...
<body class="page" onload="window.print()">
    ...
    <table class="table">
        ...
        <tr>
            <td width="50%">Tanggal : {{ date('d-m-Y', strtotime($purchase['inv_trx']->inv_trx_date)) }}</td>
            <td width="50%">Tanggal PO : {{ date('d-m-Y', strtotime($purchase['created_at'])) }}</td>
        </tr>
        <tr>
            <td width="50%">Nomor Surat Jalan : {{ $purchase['inv_trx'] != null ? $purchase['inv_trx']->no_surat_jalan : '' }}</td>
    ...
    <table class="table bordered">
        ...
        @if(count($purchase_d) > 0)
            ...
            @if($purchase_d[$i]->m_items->m_group_item_id == null)
            <tr>
