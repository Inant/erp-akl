...
class MenuController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach($menus as $menu){
            ...
            if($menu->is_page_id!=null)
                $page = DB::table('menus')->where('id',$menu->is_page_id)->first()->title;
            else
                $page = '-';
    ...
    public function addPost(Request $request)
    {
        DB::table('menus')->insert(
            [
                ...
                'is_page_id' => $request->input('is_page')=='' ? null : $request->input('is_page'),
    ...
    public function edit($id)
    {
        ...
        if($menu_by_id->is_page_id!=null){
            $page = DB::table('menus')->select('id','title')->where('id',$menu_by_id->is_page_id)->first();
            $data['page'] = $page;
        }
        return view('pages.setting.menu.menu_form', $data);
    }
    ...
    function menuByMainMenu(){
        $menus = DB::table('menus')->select('id','title')->where('is_main_menu',$_GET['is_main_menu'])->get();
        if(count($menus)>0){
            echo "<option value=''>Select Is Page</option>";
