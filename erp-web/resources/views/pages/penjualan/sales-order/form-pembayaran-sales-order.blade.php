    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-5 align-self-center">
                <Sales class="page-title">Form Pembayaran Sales Order</h4>
                <div class="d-flex align-items-center">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ URL::to('penjualan/sales-order') }}">Form Pembayaran Sales Order</a>
                            </li>
...
@section('content')
    <form method="POST" class="form-horizontal" action="{{ URL::to('penjualan/pembayaran-sales-order/store-pembayaran') }}">
        @csrf
                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <input type="hidden" name="sales_order_id" id="sales_order_id" value="{{ $salesOrder->id }}">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    <label>Customer</label>
                                    <select id="customer_id" name="customer_id" required
                                        class="form-control select2 custom-select" style="width: 100%; height:32px;"
                                        data-selected_customer="{{ $salesOrder->customer_id }}" disabled>
                                    </select>
                                </div>
                                <div class="form-group col-md-6">
                                    <label class="mb-3">Status Alamat Pengiriman</label>
                                    <br>
                                    <input type="checkbox" name="status_alamat" id="status_alamat" value="" {{$salesOrder->sesuai_alamat_customer == true ? 'checked' : ''}} disabled> <label
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <textarea name="alamat_kirim" id="alamat_kirim" class="form-control" readonly
                                        required>{{ $salesOrder->alamat_kirim }}</textarea>
                                </div>
                                <div class="form-group col-md-6">
                                    <label>Tipe Customer</label>
                                    <input type="text" name="tipe_customer" id="tipe_customer"
                                        value="{{ $salesOrder->tipe_customer }}" class="form-control" readonly>
                                </div>
                                <div class="form-group col-md-6">
                                    <label>Tanggal</label>
                                    <input type="date"
                                        value="{{ date('Y-m-d', strtotime($salesOrder->tanggal)) }}"
                                        name="tanggal" required readonly class="form-control">
                                </div>
                                <div class="form-group col-md-6">
                                    <label>Payment Method</label><br>
                                    <select id="payment_method" name="payment_method" required disabled
                                        class="form-control select2 custom-select" style="width: 100%; height:32px;">
                                        <option value="">-- Choose Payment Method --</option>
                                        <option value="cash" {{$salesOrder->payment_method == 'cash' ? 'selected' : '' }}>Cash</option>
                                        <option value="credit" {{$salesOrder->payment_method == 'credit' ? 'selected' : '' }}>Credit</option>
                                    </select>
                                </div>
                                            ...
                                            <input readonly id="stok_site[]" name="stok_site[]" type="number" class="form-control text-right" value=sto" />
                                                ...
                                                <td>
                                                    <input type="number" id="price[]" name="price[]" step="any" min="0"
                                                        class="form-control text-right" placeholder="0" required readonly
                                ...
                                <div class="col-lg-3 offset-6">
                                    <div class="float-right">
                                        ...
                                        <input type="text" readonly name="total" id="total" class="form-control"
                                            style="height:50px; font-size:28px" value="{{ $salesOrder->total }}">
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                <a href="{{ URL::to('penjualan/pembayaran-sales-order') }}" class="btn btn-danger  mb-2">Cancel</a>
    ...
    <script>
        ...
        var temp_alamat = `{!! $salesOrder->alamat_kirim !!}`;
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                url: "{{ URL::to('penjualan/sales-order/get-item-by-penawaran') }}/" +
                    {{ $salesOrder->id }}, //json get material
                dataType: 'json',
                async: false,
                success: function(response) {
        ...
        $('#addRow').on('click', function() {
            if ($('#tipe_customer').val() == '') {
                ...
                $.ajax({
                    type: "get",
                    url: "{{ URL::to('penjualan/pembayaran-sales-order/get-item') }}?id="+$('#sales_order_id').val(),
                    dataType: "json",
                    success: function(response) {
