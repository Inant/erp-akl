...
$total_tagihan=$tagihan['total_installation'];
?>
</style>
                ...
                <div class="card-body">
                    ...
                    <table style="width:100%">
                        <thead>
                            <tr>
                                <td>Nomor SPK</td>
                                <td>:</td>
                                <td>{{$install_order->spk_number}}</td>
                                <td>Nomor</td>
                                <td>:</td>
                                <td>{{$install_order->order_no}}</td>
                            </tr>
                            <tr>
                                <td>Customer</td>
                                <td>:</td>
                            ...
                            <tr>
                                <td>Tanggal Order</td>
                                <td>:</td>
                                <td>{{date('d-m-Y', strtotime($install_order->order_date))}}</td>
                                <td></td>
                                <td></td>
                                <td></td>
                            </tr>
                    ...
                    <div class="table-responsive">
                        ...
                        <thead>
                            ...
                            <tbody>
                                @foreach($install_order_d as $value)
                                <tr>
                                    <td>{{$value['item']}}</td>
                                    <td>{{$value['name']}}</td>
                                    <td>{{$value['series']}}</td>
                                    ...
                                    <td>{{formatNumber(($value['total'] * $value['installation_fee']))}}</td>
                                </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                    ...
                    @endif
                    <h4 class="card-title" {{$install_order->paid_off_date != '' ? 'hidden' : ''}}>Tambah Addendum</h4>
                    <form method="POST" action="{{ URL::to('order/bill_other') }}" class="mt-4" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="install_order_id" value="{{$install_order->id}}">
                        <input type="hidden" name="customer_id" value="{{$install_order->customer_id}}">
                        <div class="row" {{$install_order->paid_off_date != '' ? 'hidden' : ''}}>
                            <div class="col-sm-6">
                    ...
                    <hr>
                    <form method="POST" action="{{ URL::to('order/bill_install') }}" class="mt-4" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="install_order_id" value="{{$install_order->id}}">
                        <input type="hidden" name="customer_id" value="{{$install_order->customer_id}}">
                        ...
                        <h4 class="card-title"  {{$install_order->paid_off_date != '' ? 'hidden' : ''}}>Tambahkan Tagihan</h4>
                        <div class="row" {{$install_order->paid_off_date != '' ? 'hidden' : ''}}>
                            <div class="col-sm-6">
                                <div class="form-group">
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <table>
                    <tr>
                        <td>Total Tagihan (Include PPN & PPH 23)</td>
                ...
                <input type="hidden" id="total_pph" name="total_pph">
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label for="">Total</label>
...
function persenTotal(eq){
    ...
    if (total_persen > parseFloat(sub_total)) {
        ...
        pph=(parseFloat(total_persen)*0.02).toFixed(0);
        $('#pph_bill').val(formatNumber(pph))
        bill_amount=parseFloat(total_persen) + (parseFloat(ppn) - parseFloat(pph));
...
function doShowDetail(eq){
    ...
    $('#amount_bill').val(parseFloat(amount) + parseFloat(ppn) + parseFloat(pph));
    $('#bill_amount').html(': '+formatRupiah(parseFloat(amount)));
    // $('#bill_ppn').html(': '+formatRupiah(parseFloat(ppn)));
    // $('#bill_pph').html(': '+formatRupiah(parseFloat(pph)));
    $('#bill_all').html(': '+formatRupiah(parseFloat(amount) + parseFloat(ppn) + parseFloat(pph)));
    $('#total_min').val((parseFloat(amount) + parseFloat(ppn) + parseFloat(pph)) - parseFloat(total_min))
}
function checkTotalBill(eq){
    ...
    $('#total_bill').val(formatNumber(paid));
    var paid_more=0;
    if (total_paid >= parseFloat(sub_total)) {
