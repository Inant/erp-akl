			...
			<div class="page-breadcrumb">
                <div class="row">
                    <div class="col-5 align-self-center">
                        <h4 class="page-title">Pembayaran Tagihan Customer</h4>
    ...
    <div class="row">
        ...
        <div class="col-12">
            <div class="text-right">
                <a href="{{ URL::to('inventory/paid_customer') }}"><button class="btn btn-success btn-sm mb-2"><i class="fas fa-plus"></i>&nbsp; Create New</button></a>
            </div>
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Pembayaran Tagihan Customer</h4>
                        ...
                        <table id="paid_list" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">No Tagihan Customer</th>
            ...
            <div class="modal-body">
                <div class="table-responsive">
                    <table id="listDetailAll" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                <th class="text-center">No Hutang</th>
                                <th class="text-center">No Faktur</th>
...
$(document).ready(function() {
    $('#paid_list').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "{{ URL::to('/inventory/list_paid_customers') }}",
        'aaSorting' : [3, 'desc'],
        "columns": [
            {"data": "no", "class" : "text-center"},
            {"data": "no_payment", "class" : "text-center"},
    ...
    $('#listDetailAll').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "{{ URL::to('inventory/list_detail_paid_cust') }}",
        'aaSorting' : [3, 'desc'],
        "columns": [
            {"data": "bill_no", "class" : "text-center"},
            {"data": "faktur_no", "class" : "text-center"},
...
function doShowDetail(eq){
    ...
    t2.clear().draw(false);
    $.ajax({
        url: "{{ URL::to('inventory/list_paid_customers_d') }}"+'/'+eq,
        dataType : 'json',
        success: function(response){
            ...
            for(i = 0; i < arrData.length; i++){
                t2.row.add([
                    ...
                    '<div class="text-left">'+arrData[i]['bill_no']+'</div>',
                    '<div class="text-left">'+arrData[i]['invoice_no']+'</div>',
                    '<div class="text-right">'+formatCurrency(parseInt(arrData[i]['amount']))+'</div>',
                    '<div class="text-center"><a '+(arrData[i]['order_id'] != null ? 'href="{{URL::to('order/print_kwitansi/')}}/'+arrData[i]['customer_bill_d_id']+'"' : 'href="{{URL::to('order/print_kwitansi_install/')}}/'+arrData[i]['customer_bill_d_id']+'"')+ 'class="btn btn-success btn-sm" target="_blank"><i class="mdi mdi-printer"></i></a></div>'
                ]).draw(false);
            }
        }
    });
