...
<table border="1">
    <thead>
        <tr>
            <th colspan="11">Laporan Piutang dan Pembayaran Customer per Tanggal {{formatDate($data['date1'])}} - {{formatDate($data['date2'])}}</th>
        </tr>
    </thead>
    <thead>
        <tr>
    ...
    @endphp
        ...
        @php 
        $sub_total=($value->tipe == 'DEBIT' ? (round($sub_total)+round($value->jumlah)) : (round($sub_total)-round($value->jumlah)));
        $total_debit+=($value->tipe == 'DEBIT' ? round($value->jumlah) : 0);
        $total_kredit+=($value->tipe == 'KREDIT' ? round($value->jumlah) : 0);
        $n++;

        $getBillNo = \DB::table('inv_sales')->where('id', $value->inv_sale_id)->select('bill_no');
        ...
        @if(isset($detailBill) && count($detailBill) > 0 && $value->source != null && strpos($value->deskripsi, ','))
            <tr style="vertical-align: middle; text-align: center">
                ...
                <td rowspan="{{$cekKurangLebihBayar > 0 ? count($detailBill) + 1 : count($detailBill)}}">{!!$value->customer_bill_no != null ? $value->customer_bill_no : ($value->order_no != null ? $value->order_no : ($value->install_order_no != null ? $value->install_order_no : ( $getBillNo->count() > 0 ? $getBillNo->first()->bill_no : '-')))!!}</td>
                ...
                <td rowspan="{{$cekKurangLebihBayar > 0 ? count($detailBill) + 1 : count($detailBill)}}">{{round($sub_total)}}</td>
                @php
        ...
        @else
            ...
            @if ($cekKurangLebihBayar > 0)
                <tr style="vertical-align: middle; text-align: center">
                    ...
                    <td rowspan="2">{!!$value->customer_bill_no != null ? $value->customer_bill_no : ($value->order_no != null ? $value->order_no : ($value->install_order_no != null ? $value->install_order_no : ( $getBillNo->count() > 0 ? $getBillNo->first()->bill_no : '-')))!!}</td>
                    <td rowspan="2">
                    @if($value->source != null){{$value->source}}@else - 
                    @endif
                    </td>
                    ...
                    <td rowspan="2">{{round($sub_total)}}</td>
                    {{-- jika terdapat kurang / lebih bayar --}}
                    @if ($cekKurangLebihBayar > 0 && $nominalKurangLebih > 0)
                    <tr>
                        <td>Lebih Bayar {{$value->source}}</td>
                        ...
                        @php
                            $noTagDibayar = '-';
                            if ($value->source != null && $value->tipe == 'KREDIT') {
                                $noTagDibayar = explode('No Tagihan', $value->deskripsi)[1];
                            }
                        @endphp
                    <td>{{$noTagDibayar}}</td>
            ...
            @else
                <tr style="vertical-align: middle; text-align: center">
                    ...
                    <td>{!!$value->customer_bill_no != null ? $value->customer_bill_no : ($value->order_no != null ? $value->order_no : ($value->install_order_no != null ? $value->install_order_no : ( $getBillNo->count() > 0 ? $getBillNo->first()->bill_no : '-')))!!}</td>
                    <td>
                    @if($value->source != null){{$value->source}}@else - 
                    @endif
                    </td>
    ...
    @endforeach
        <tr>
            ...
            <th>{{round($total_debit)}}</th>
            <th>{{round($total_kredit)}}</th>
            <th>{{round($sub_total)}}</th>
        </tr>
    </tbody>
</table>
<?php
header("Content-type: application/vnd-ms-excel");
header("Content-Disposition: attachment; filename=laporan_piutang_pembayaran_customer.xls");
?>