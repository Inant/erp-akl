...
<body class="page" onload="window.print()">
    ...
    <table class="table">
        <tr>
            ...
            <td width="50%">Nomer Transfer : {{ $transfer_stock['no'] }}</td>
        </tr>
        <tr>
            <td width="50%">Tanggal : {{ date('d-m-Y') }}</td>
            <td width="50%">Tanggal Transfer : {{ date('d-m-Y', strtotime($transfer_stock['created_at'])) }}</td>
        </tr>
        <tr>
            <td width="50%"></td>
            <td width="50%">Dari Gudang : {{ $transfer_stock['sites']['name'] }}</td>
        </tr>
    </table>
    ...
    <table class="table bordered">
        ...
        @if(count($transfer_stock_d) > 0)
            @for($i = 0; $i < count($transfer_stock_d); $i++)
            <tr>
                <td class="text-center">{{ ($i + 1) }}</td>
                <td class="text-center">{{ $transfer_stock_d[$i]['m_items']['no'] }}</td>
                <td>{{ $transfer_stock_d[$i]['m_items']['name'] }}</td>
                <td class="text-right">{{ (float)$transfer_stock_d[$i]['amount'] }}</td>
                <td class="text-center">{{ $transfer_stock_d[$i]['m_units']['name'] }}</td>
                <td>{{ $transfer_stock_d[$i]['notes'] }}</td>
            </tr>
            @endfor
        @endif
    </table>
