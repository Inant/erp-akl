        ...
        <div class="col-5 align-self-center">
            ...
            <div class="d-flex align-items-center">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ URL::to('po_asset_ao') }}">PO Autorisasi</a></li>
...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <form method="POST" action="{{ URL::to('po_konstruksi/approve_asset') }}" class="form-horizontal r-separator">
                @csrf
                <input type="hidden" name="purchase_id" value="{{ $purchase['id'] }}" />
                <div class="text-right">
                    <a href="{{ URL::to('po_konstruksi/po_asset_ao') }}"><button type="button" class="btn btn-danger btn-sm mb-2">Cancel</button></a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            type: "GET",
            url: "{{ URL::to('po_konstruksi/detail_atk/' . $purchase['id']) }}", //json get site
            dataType: 'json',
            async: false,
    ...
    saveSignatureButton1.addEventListener("click", function(event) {
        if (signaturePad1.isEmpty()) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ URL::to('po_konstruksi/acc_ao_asset_form/signature_holding/' . $purchase['id']) }}",
                dataType: 'json',
                data: {
                    _token: CSRF_TOKEN,
                    file: dataURL
    ...
    saveSignatureButton2.addEventListener("click", function(event) {
        if (signaturePad2.isEmpty()) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ URL::to('po_konstruksi/acc_ao_asset_form/signature_supplier/' . $purchase['id']) }}",
                dataType: 'json',
                data: {
                    _token: CSRF_TOKEN,
                    file: dataURL
    ...
    function saveSignatureDirector() {
        ...
        $.ajax({
            type: "POST",
            url: "{{ URL::to('po_konstruksi/saveATKSignatureDirector/' . $purchase['id']) }}", //json get site
            dataType: 'json',
            data: {
                _token: CSRF_TOKEN,
                code: code
    ...
    function saveSignatureManager() {
        ...
        $.ajax({
            type: "POST",
            url: "{{ URL::to('po_konstruksi/saveATKSignatureManager/' . $purchase['id']) }}", //json get site
            dataType: 'json',
            data: {
                _token: CSRF_TOKEN,
                code: code
