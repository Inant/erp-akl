            ...
            <div class="col-5 align-self-center">
                ...
                <div class="d-flex align-items-center">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item active" aria-current="page">Master Material</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="text-right">
                <a href="{{ URL::to('master_material/create') }}"><button class="btn btn-success btn-sm mb-2"><i class="fas fa-plus"></i>&nbsp; Create New</button></a>
            </div>
            <div class="card">
                <div class="card-body">
                        ...
                        <table id="material_list" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Item Group Set</th>
                                    <th class="text-center">Total Item In Set</th>
...
$(document).ready(function() {
    $('#material_list').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "{{ URL::to('master_material/list') }}", //json get site
        aaSorting: [[6, 'desc']],
        "columns": [
            {"data": "no"},
            {"data": "name"},
            ...
            {"data": "id",
                "render" : function(data, type, row){ return row.item_set != null ? row.item_set.name : '-'}
            },
            {"data": "amount_in_set",
                "render" : function(data, type, row){ return row.amount_in_set != null ? row.amount_in_set : '-'}
            },
            {"data": "id", 
            "render" : function(data, type, row){ return '<div class="text-center"><a href="{{ URL::to('master_material/edit') }}/'+row.id+'" class="btn waves-effect waves-light btn-xs btn-warning"><i class="fas fa-pencil-alt"></i></a> <a href="{{ URL::to('master_material/delete') }}/'+row.id+'" class="btn waves-effect waves-light btn-xs btn-danger" onclick="javasciprt: return confirm(\'Are You Sure ?\')"><i class="fas fa-trash-alt"></i></a></div>'}
            }
        ],
    } );
});
