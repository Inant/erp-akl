			...
			<div class="page-breadcrumb">
                <div class="row">
                    <div class="col-5 align-self-center">
                        <h4 class="page-title">Pembayaran Tagihan Pemasok</h4>
    ...
    <div class="row">
        ...
        <div class="col-12">
            <div class="text-right">
                <a href="{{ URL::to('inventory/paid_supplier') }}"><button class="btn btn-success btn-sm mb-2"><i class="fas fa-plus"></i>&nbsp; Create New</button></a>
            </div>
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Pembayaran Tagihan Pemasok</h4>
                    <div class="form-group float-right">
                        <button class="btn btn-primary" onclick="listDetail()" data-toggle="modal" data-target="#modalListDetail">List Detail</button>
                        <a href="{{ URL::to('inventory/export_paid_supplier') }}" target="_blank" class="btn btn-success">Export</a>
                    </div>
...
$(document).ready(function() {
    $('#paid_list').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "{{ URL::to('/inventory/list_paid_supplier') }}",
        'aaSorting' : [3, 'desc'],
        "columns": [
            {"data": "no", "class" : "text-center"},
            {"data": "dt", "class" : "text-center"},
    ...
    $('#listDetailAll').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "{{ URL::to('inventory/list_detail_paid_sppl') }}",
        'aaSorting' : [3, 'desc'],
        "columns": [
...
function doShowDetail(eq){
    ...
    t2.clear().draw(false);
    $.ajax({
        url: "{{ URL::to('inventory/list_paid_supplier_d') }}"+'/'+eq,
        dataType : 'json',
        success: function(response){
            ...
            for(i = 0; i < arrData.length; i++){
                t2.row.add([
                    ...
                    '<div class="text-left">'+arrData[i]['paid_no']+'</div>',
