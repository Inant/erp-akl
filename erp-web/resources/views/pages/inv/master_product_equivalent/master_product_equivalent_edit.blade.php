        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ URL::to('master_product_equivalent/edit/' . $id ) }}" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group row">
                            <label class="col-sm-3 text-right control-label col-form-label">Code</label>
                            <div class="col-sm-8">
                                <input type="text" class="form-control" name="code" value="{{ $m_products->code }}" required />
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-sm-3 text-right control-label col-form-label">Name</label>
                            <div class="col-sm-8">
                                <input type="text" class="form-control" name="name" value="{{ $m_products->name }}" required />
                            </div>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table id="dt_temp" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    @foreach($m_product_ds as $value)
                                    <tr>
                                        <td>
                                            <select name="m_item_id[]" class="form-control select2" style="width: 100%;">
                                                <option value="">-- Select Material --</option>
                                                @foreach ($m_items as $value2)
                                                @if ($value->m_item_id == $value2->id)
                                                    <option selected value="{{ $value2->id }}">({{ $value2->no }}) {{ $value2->name }}</option>
                                                @else
                                                    <option value="{{ $value2->id }}">({{ $value2->no }}){{ $value2->name }}</option>
                                                @endif
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
                                            <select name="dimensi[]" class="form-control select2" style="width: 100%;">
                                                <option value="None" {{ explode(';', $value->formula)[0] == 'None' ? 'selected' : '' }}>None</option>
                                                <option value="H" {{ explode(';', $value->formula)[0] == 'H' ? 'selected' : '' }}>H</option>
                                                <option value="W" {{ explode(';', $value->formula)[0] == 'W' ? 'selected' : '' }}>W</option>
                                            </select>
                                            <!-- <input name="dimensi[]" type="text" class="form-control text-left" value="{{ explode(';', $value->formula)[0] }}" /> -->
                                        </td>
                                        <td>
                                            <select name="operator[]" class="form-control select2" style="width: 100%;">
                                                <option value="None" {{ explode(';', $value->formula)[1] == 'None' ? 'selected' : '' }}>None</option>
                                                <option value="-" {{ explode(';', $value->formula)[1] == '-' ? 'selected' : '' }}>-</option>
                                                <option value="+" {{ explode(';', $value->formula)[1] == '+' ? 'selected' : '' }}>+</option>
                                            </select>
                                            <!-- <input name="operator[]" type="text" class="form-control text-left" value="{{ explode(';', $value->formula)[1] }}"  /> -->
                                        </td>
                                        <td>
                                            <input name="equivalent[]" onkeyup="handleEquivalent()" type="text" class="form-control text-left" value="{{ explode(';', $value->formula)[2] }}"  />
                                        </td>
                                        <td>
                                            <input name="qty_item[]" type="number" class="form-control text-left" min="0" value="{{ $value->qty_item }}" />
                                        </td>
                                        <!-- '<td>'+
                        '<select name="m_item_id[]" class="form-control select2"></select>' +
                    '</td>'+
                    '<td>'+
                        '<select name="dimensi[]" class="form-control select2"><option value="None">None</option><option value="H">H</option><option value="W">W</option></select>' +
                    '</td>'+
                    '<td>'+
                        '<select name="operator[]" class="form-control select2"><option value="None">None</option><option value="-">-</option><option value="+">+</option></select>' +
                    '</td>'+
                    '<td>'+
                        '<input name="equivalent[]" type="number" class="form-control text-left" min="0" />' +
                    '</td>'+
                    '<td>'+
                        '<input name="qty_item[]" type="number" class="form-control text-left" min="0" />' +
                    '</td>'+ -->
...
function handleEquivalent() {
    countMaterial = $('[name^=m_item_id]').length;
    var letters = /^[A-Za-z]+$/;
    for(i = 0; i < countMaterial; i++){
        formEquivalent = $('[name^=equivalent]').eq(i).val();
        
        let lastChar = formEquivalent.substr(formEquivalent.length - 1);
        if(lastChar.match(letters)) {
            let equivalentVal = '';
            $.each(arrEquivalent, function(i, item) {
                if(lastChar.toLowerCase().includes(arrEquivalent[i]['code'].toLowerCase())) {
                    equivalentVal = formEquivalent;
                }
            });
            $('[name^=equivalent]').eq(i).val(equivalentVal);
        }
    }
}
